...
class TuckerMatrix(Tucker):
    ...
    def from_dense(
        cls,
        dense_tensor: back.type(),
        n: Union[Sequence[int], None] = None,
        m: Union[Sequence[int], None] = None,
        eps=1e-14,
    ):
        """Convert dense tensor into `TuckerMatrix` representation.
        The resul is tensor that represents linear operator
        from R^N to R^M.
    ...
    def __matmul__(self, other: Union[back.type(), Tucker, "TuckerMatrix"]):
        """Perform matrix multiplication of tensors.
        If other's ndim > matrix ndim, then perform batch matmul over last
        ndim modes.

        :param other: Can be dense tensor, Tucker or TuckerMatrix.
         If dense tensor, then treated as vector and matvec operation is performed.
         The result is also a dense tensor. If `Tucker`,
         then treated as vector and matvec
         operation is performed. The result is also a Tucker.
         Else matmul operation is performed and another matrix is returned.
        :return: Dense tensor, Tucker or TuckerMatrix, depends on type of `other`.
        """
        if isinstance(other, back.type()):
            ...
            for i in range(self.ndim):
                reshaped_factors.append(
                    back.reshape(self.factors[i], (self.n[i], self.m[i], -1), order="F")
                )
                factors_letters.append(
                    ascii_letters[self.ndim + 2 * i : self.ndim + 2 * (i + 1)]
                    + core_letters[i]
                )
            ...
            einsum_eq = (
                f"{core_letters},"
                f"{','.join(factors_letters)},"
                f"{''.join(operand_letters)}->"
                f"{''.join(result_letters)}"
            )
