...
class SparseTensor:
    """
    Contains sparse tensor in coo format.
    It can be constructed manually or converted from dense format.
    """

    def __init__(
        self, shape: Sequence[int], inds: Sequence[Sequence[int]], vals: Sequence[float]
    ):
        """
        Parameters
        ----------
    ...
    def dense2sparse(cls, T: back.type()):
        ...
        grid_inds = tuple(
            i.flatten(order="F") for i in np.meshgrid(*inds, indexing="ij")
        )
    ...
    def __unfolding_to_tensor(unfolding: csr_matrix, k: int, shape: Sequence[int]):
        ...
        def detach_multiindex(p, multiindex):
            ...
            """
            inds = []
            dynamic = np.zeros_like(
                multiindex[1]
            )  # use dynamic programming to prevent d**2 complexity
    ...
    def reshape(self, new_shape: Sequence[Sequence[int]]):
        ...
        ----------
         new_shape : Sequence[Sequence[int]]
            Maps old tensor shape to a new one.
             For example, if tensor was of order 3 of shape (512, 512, 3), then
             each of its element had 3 indices (i, j, k).
             Now we want to reshape it to tensor of order 5 of shape (64, 8, 64, 8, 3).
             Then `new_shape` parameter will look like [[64, 8], [64, 8], [3]].
        """
        ...
        for i, shape in enumerate(new_shape):
            ...
            new_inds = (
                new_shape_inds
                if new_inds is None
                else back.concatenate((new_inds, new_shape_inds))
            )
    ...
    def contract(self, contraction_dict: Dict[int, back.type()]):
        ...
        contraction_dict: dict[int, back.tensor]
            dictionary of pairs (i, M), where i is mode index, and M is matrix,
             we want to contract tensor with by i mode
