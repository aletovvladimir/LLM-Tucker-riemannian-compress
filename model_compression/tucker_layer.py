...
class TuckerLinear(RiemannLayer):
    ...
    def _decompose(self, layer: nn.Linear, rank) -> TuckerMatrix:
        ...
        data = data.permute(
            [i // 2 if i % 2 == 0 else i // 2 + n for i in range(2 * n)]
        )
        data = data.reshape(self.dims[1])
        matrix = TuckerMatrix.from_dense(
            data, self.dims[0][:3], self.dims[0][3:], self.eps
        )
    ...
    def forward(self, x):
        ...
        matrix = TuckerMatrix(
            self.core, self.factors, self.dims[0][:3], self.dims[0][3:]
        )
        res = matrix @ x_wav
